<template>
  <div class="main">
    <div>
        ...
        <el-row :gutter="20">
          ...
          <el-col :span="8">
            <el-form-item label="订单状态">
              ...
              <!-- <el-input v-model="orderForm.orderStatus"></el-input> -->
            </el-form-item>
          </el-col>
          <el-col :span="8">
            <el-form-item label="付款方式">
              <el-select v-model="orderForm.payWay">
                <el-option
                  v-for="item in payWayList" :key="item"
                  :label="item"
                  :value="item">
                </el-option>
              </el-select>
              <!-- <el-input v-model="orderForm.payWay"></el-input> -->
            </el-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="20">
      ...
      <page-title class="page-title" title="处方订单列表" style="margin-bottom: 20px;"></page-title>
      <el-table :data="orderList" border v-loading="loading" stripe>
        ...
        <el-table-column width="180" property="prescriptionNo" label="处方号"></el-table-column>
        <el-table-column property="personName" label="患者姓名"></el-table-column>
        <el-table-column property="secondDepartment" label="挂号科室"></el-table-column>
        <el-table-column property="userRealName" label="医师姓名"></el-table-column>
        <el-table-column property="orderPrice" label="金额(元)"></el-table-column>
        ...
        <el-table-column label="支付状态">
          <template slot-scope="scope">
            ...
            <el-tag type="primary" v-else-if="scope.row.orderStatus === '0'">已创建</el-tag>
            <el-tag type="info" v-else>已过期</el-tag>
          </template>
        </el-table-column>
        <el-table-column width="150" property="createTime" label="创建时间"></el-table-column>
        <el-table-column width="150" property="payTime" label="支付时间"></el-table-column>
        <el-table-column property="orderRemark" label="订单备注"></el-table-column>
  ...
  methods: {
    deleteReg(info) {
      this.$confirm('删除记录可能会导致重要数据丢失，是否继续?', '提示', {
        ...
        this.$postJson(`/reg/system/prescription/deleteOrder`, {orderNo: info.orderNo}).then(res => {
    ...
    initPage() {
      ...
      this.$postJson('/reg/system/prescription/orderList', params).then(res => {
