  ...
  <div class="main">
    <div>
      ...
      <el-table :data="drugList" v-loading="loading" stripe @selection-change="handleSelectionChange">
        ...
        <el-table-column width="300" property="drugIndication" show-overflow-tooltip label="功能主治"></el-table-column>
      ...
      <el-dialog top="5vh" class="dialog" :title="title" :visible.sync="drugDialog" width="60%" :before-close="handleClose">
        <el-form :close-on-click-modal="false" label-width="110px" :model="drugInfo" class="drugInfoForm" ref="drugref" :rules="rules">
          ...
          <el-row :gutter="20">
            <el-col :span="24">
              <el-form-item label="功能主治:" prop="drugIndication"><el-input :rows="4" type="textarea" v-model="drugInfo.drugIndication"></el-input></el-form-item>
            </el-col>
          </el-row>
          <el-row :gutter="20">
            <el-col :span="24">
      ...
      <el-dialog class="dialog" top="5vh" title="药品详情" :visible.sync="drugInfoDialog" width="70%" :before-close="handleInfoClose">
        ...
        <el-descriptions :column="1" border>
          <el-descriptions-item :labelStyle="{'width' : '100px'}">
            <template slot="label">功能主治</template><span v-html="drugInfo.drugIndication"></span>
          </el-descriptions-item>
        </el-descriptions>
        <el-descriptions :column="1" border>
          <el-descriptions-item :labelStyle="{'width' : '100px'}">
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        drugIndication: [{ required: true, message: '请输入功能主治', trigger: 'blur' }],
  ...
  methods: {
    ...
    // 导出
    exportExcel() {
      htmlToExcel.getExcel('#selectTable','中成药')
    },
    // 关闭导出预览弹窗
    handleExportClose() {
    ...
    // 获取中成药作用类别列表
    getDrugTypeList() {
      this.$get(`/reg/system/drugType/list/${'2'}`).then((res) => {
        let resData = res.data
        if(resData.code === 200) {
