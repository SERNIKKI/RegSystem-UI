    ...
    <div>
      ...
      <el-table :data="patList" border v-loading="loading" stripe>
        ...
        <el-table-column align="center" label="操作">
          <template slot-scope="scope">
            <el-button type="danger" size="mini" disabled plain @click="deleteReg(scope.row)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    initPage() {
      ...
      this.patForm.endTime = this.$moment(new Date().getTime() - (1 * 24 * 60 * 60 * 1000)).format('YYYY-MM-DD')
    ...
    clearForm() {
      this.patForm = {
        ...
        endTime: this.$moment(new Date().getTime() - (1 * 24 * 60 * 60 * 1000)).format('YYYY-MM-DD')
      }
      this.initPage()
    },
